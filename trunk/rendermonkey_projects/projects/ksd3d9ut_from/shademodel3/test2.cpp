...
ksd3d9ut::shademodel3::Owner   g_owner;
	...
	ResourseIndex   g_texture_diffuseIndex ;
	ResourseIndex   g_texture_normalmapIndex;
	ResourseIndex   g_texture_additiveIndex;
	ResourseIndex   g_texture_cubemapIndex;
...
HRESULT do_test(IDirect3DDevice9* device, float time)
{
	...
	IDirect3DTexture9*  tex_diffuse    = g_rm->GetTexture2dWithSearch(g_texture_diffuseIndex, "diffuse.tga");
	IDirect3DTexture9*  tex_normalMap  = g_rm->GetTexture2dWithSearch(g_texture_normalmapIndex, "normalMap.tga");
	IDirect3DTexture9*  tex_additive    = g_rm->GetTexture2dWithSearch(g_texture_additiveIndex, "additive.tga");
	IDirect3DCubeTexture9* tex_cubemap  = g_rm->GetTextureCubeWithSearch(g_texture_cubemapIndex, "cubemap.dds");


	//static ResourseIndex riMesh;
	//ID3DXMesh* model = g_rm->GetMeshWithSearch(riMesh, "model.x");
	...
	case ShadeModelType_e::textured: {
	  hr|= g_owner.GetTextured()->SetMatrixWorldViewProj(&wvp);
	   hr|=g_owner.GetTextured()->SetTextureDiffuse(tex_diffuse);
		 ...
		 //IParameterTransfWorldViewProj,
		  hr|=g_owner.GetDiffuse()->SetMatrixWorldViewProj(&wvp);
		   ...
		   hr|=g_owner.GetDiffuse()->SetMatrixWorld(&world);
			...
			hr|=g_owner.GetDiffuse()->SetTextureDiffuse (   tex_diffuse  );
	...
	case ShadeModelType_e::phongspecular: {
		...
		hr|=g_owner.GetPhongSpecular()->SetTextureDiffuse(tex_diffuse);
	...
	case ShadeModelType_e::phongilumination: {
		...
		hr|=g_owner.GetPhongIlumination()->SetTextureDiffuse(tex_diffuse);
		hr|=g_owner.GetPhongIlumination()->SetTextureAdditive( tex_additive );
	...
	//IParameterDiffuseTexture,
	hr|=g_owner.GetBumpSpecular()->SetTextureDiffuse(tex_diffuse);

	//IParameterNormalMapTexture
	hr|=g_owner.GetBumpSpecular()->SetTextureNormalMap( tex_normalMap );
	...
	//IParameterDiffuseTexture,
	hr|=g_owner.GetBumpReflectiveSpecular()->SetTextureDiffuse( tex_diffuse);
	//IParameterNormalMapTexture,
	hr|=g_owner.GetBumpReflectiveSpecular()->SetTextureNormalMap(tex_normalMap);
	//IParameterCubemapReflectionTexture
	hr|=g_owner.GetBumpReflectiveSpecular()->SetTextureCubemapReflection(tex_cubemap);
	...
	// IParameterDiffuseTexture,
	 hr|=g_owner.GetBumpSpecularMap()->SetTextureDiffuse(tex_diffuse);

	// IParameterNormalMapTexture,
	 hr|=g_owner.GetBumpSpecularMap()->SetTextureNormalMap(tex_normalMap);

	// IParameterSpecularTexture
	hr|=g_owner.GetBumpSpecularMap()->SetTextureSpecular(tex_additive);
	...
	case ShadeModelType_e::cubemapreflection: {
		...
		hr|=g_owner.GetCubemapReflection()->SetTextureCubemapReflection( tex_cubemap );
	...
	case ShadeModelType_e:: diffusemulcubemapreflection :  {
	 ...
	 hr|=g_owner.GetDiffuseMulCubemapReflection()->SetTextureDiffuse( tex_diffuse );
	 hr|=g_owner.GetDiffuseMulCubemapReflection()->SetTextureCubemapReflection(  tex_cubemap );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if(g_pmesh==NULL)
	{
		...
		if FAILED( g_rm->Search(path, "model.x") )
		{
		...
		g_pmesh = new ksd3d9ut::dxmesh::d3dxmesh( pd3dDevice, g_loader,  
			...
			path, //"j:\\ksd3d9ut\\ksd3d9ut\\media\\teapot.x",
			true 
	...
	//IDirect3DTexture9* tex= g_rm->GetTexture2dWithSearch(g_texture_diffuseIndex, "diffuse.tga");
